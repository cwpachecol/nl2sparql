...
import learning.transformers.preprocess_lcquad10 as preprocess_lcquad10
...
import numpy as np
import itertools
from parsers.lc_quad10_linked import LC_Quad10_Linked
from parsers.qald import Qald
...
class Orchestrator:
    def __init__(self, logger, question_classifier, double_relation_classifer, parser, filepath, auto_train=True):
        ...
        self.dep_tree_cache_file_path = './caches/dep_tree_cache_lcquad10test.json'
    ...
    def rank(self, args, question, generated_queries):
        ...
        if 2 > 1:
            ...
            preprocess_lcquad10.save_split(output_dir, *preprocess_lcquad10.split(json_data, self.parser))

            if question in self.dep_tree_cache:
                preprocess_lcquad10.parse(output_dir, dep_parse=False)
                ...
                preprocess_lcquad10.parse(output_dir)
    ...
    def generate_query(self, question, entities, relations, h1_threshold=9999999, question_type=None):
        ...
        args = Struct()
        base_path = "./learning/transformers/"
        args.expname = "lc_quad10,epoch=5,train_loss=0.08340245485305786"
        args.mem_dim = 150
        args.hidden_dim = 50
        args.num_classes = 2
        args.input_dim = 300
        ...
        args.lr = 0.01
        args.wd = 1e-4
        args.data = os.path.join(base_path, "./data/lcquad10/")
        args.cuda = False
        # args.cuda = True
        try:
...
if __name__ == "__main__":
    ...
    if not os.path.isfile("./data/lcquad10/linked_test.json"):
        with open("./learning/transformers/data/lcquad10/lcquad10_test.json", 'r') as f:
            test = json.load(f)

        with open('./data/lcquad10/linked_answer.json', 'r') as f:
            data = json.load(f)
        ...
        for i in data:
            if i['id'] in test_id:
                test_data.append(i)
        with open('./data/lcquad10/linked_test.json', 'w') as f:
            json.dump(test_data, f)

    ds = LC_Quad10_Linked(path="./data/lcquad10/linked_test.json")
    ds.load()
    ds.parse()
    ...
    output_file = 'lcquad10testanswer_output'
    linker = Earl(path="./data/lcquad10/entity_lcquad10_test.json")

    # for e in linker.raw_data:
    #     print(e['question'])
    # # print(linker.raw_data)

    # for i in linker.questions:
    #     print(i[:][:])
    # print("*****"*50)
    # print(linker.questions)
    ...
    with open('na_list_lcquad10test.txt', 'w') as f:
        for i in na_list:
            f.write("{}\n".format(i))